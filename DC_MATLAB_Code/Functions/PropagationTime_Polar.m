...
elseif HeadLine == 1
    ...
    if  Mode == 1
        ax.Title.String = ['$t_{\mathrm{Prop}}$ for $c_{\mathrm{e}1}$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
    elseif Mode == 2
        ax.Title.String = ['$t_{\mathrm{Prop}}$ for $|\vec{c}_{\mathrm e}|$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
    end
elseif SymmetricSystem == 0 && HeadLine == 2
    ax.Title.FontSize = FontSizeHeadLine;
    if  Repetitions == 1
        if  Mode == 1
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $c_{\mathrm{e}1}$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,'] @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        elseif Mode == 2
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $|\vec{c}_{\mathrm e}|$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,'] @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        end
    elseif Repetitions > 1
        if  Mode == 1
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $c_{\mathrm{e}1}$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        elseif Mode == 2
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $|\vec{c}_{\mathrm e}|$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        end
    end
elseif SymmetricSystem == 1 && HeadLine == 2
    ...
    if  Repetitions == 1
        if  Mode == 1
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $c_{\mathrm{e}1}$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{\mathrm s}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        elseif Mode == 2
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $|\vec{c}_{\mathrm e}|$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{\mathrm s}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        end
    elseif Repetitions > 1
        if  Mode == 1
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $c_{\mathrm{e}1}$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'\mathrm s}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        elseif Mode == 2
            ax.Title.String = ['$t_{\mathrm{Prop}}$ for $|\vec{c}_{\mathrm e}|$ in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'\mathrm s}$ @ ',num2str(Frequency),'\,kHz and ',num2str(Distance),'\,mm'];
        end
    end
end
z = ax.RTick;
    ...
    try
        if  PDF == 1
            if  Crop == 0
                ...
                if  Mode == 1
                    print(f,fullfile(Directory,[FileName,'_tce1']),'-dpdf')
                elseif Mode == 2
                    print(f,fullfile(Directory,[FileName,'_tceAbs']),'-dpdf')
                end
            elseif Crop == 1
                if  Mode == 1
                    exportgraphics(f,fullfile(Directory,[FileName,'_tce1.pdf']))
                elseif Mode == 2
                    exportgraphics(f,fullfile(Directory,[FileName,'_tceAbs.pdf']))
                end
            end
        end
        if  PNG == 1
            if  Crop == 0
                if  Mode == 1
                    print(f,fullfile(Directory,[FileName,'_tce1']),'-dpng',['-r',num2str(PNGresolution)])
                elseif Mode == 2
                    print(f,fullfile(Directory,[FileName,'_tceAbs']),'-dpng',['-r',num2str(PNGresolution)])
                end
            elseif Crop == 1
                if  Mode == 1

...
% =========================================================================
function ALamb = Computer_Isotropic_ALamb_F(Multithreading,Q1,Q2,ax,Viscoelastic,FluidLoading,Fluid,Material,FrequencyRange,Half,HigherOrderModes,FALambF,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,ASteps,AOffset,MissingSamples,BelowCutoffWidth)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS>
global Stop
            ...
            for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeReal X SweepRangeImag)
                ...
                for o = 1:SearchAreaSections % increase search resolution
                    ...
                    for k = 1:1e2 % search minimum in characteristic equation and converge upon it 
                        ...
                        for j = 2:size(Y,1)-1
                            if  SweepRangeReal(j-1) > Material.LongitudinalVelocity && SweepRangeReal(j+1) < Material.LongitudinalVelocity
                                Y(j,:) = NaN;
                            end
                        end
...
% end
        ...
        if  max(ALamb{p+1}(:,4))*1e3 > PhaseVelocityLimit
            ...
            PhaseVelocityRange = Max+ASteps:ASteps:PhaseVelocityLimit+ASteps;
            for i = 1:length(PhaseVelocityRange)
                ...
                for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeFrq X SweepRangeImag)
                    if  i == 1
                        SweepRangeFrq = [ALamb{p+1}(MaxInd,1)-AOffset/Half/2e3 ALamb{p+1}(MaxInd,1)+4/Half/2e3];
                        SweepRangeImag = [SearchWidthImag(1)*ALamb{p+1}(MaxInd,8) SearchWidthImag(2)*ALamb{p+1}(MaxInd,8)];
                    else
                        SweepRangeFrq = [X1{p+1}(end-1,1)-AOffset/Half/2e3 X1{p+1}(end-1,1)+4/Half/2e3];

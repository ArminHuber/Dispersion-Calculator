...
for n = 1:length(PropagationAngle)
    if  A0 == 1
        X(n,1) = 2*pi*Frequency/A{n,1}(q,1);
    end
    if  SH0 == 1
        X(n,2) = 2*pi*Frequency/A{n,2}(q,1);
    end
    if  S0 == 1
        X(n,3) = 2*pi*Frequency/A{n,3}(q,1);
    end
end
...
f = figure('Name','Wavenumber','Toolbar','none','Units','normalized','OuterPosition',[0 0 .6 1],'color','w');
datacursormode on
...
elseif HeadLine == 1
    ...
    ax.Title.String = ['Wavenumber in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' @ ',num2str(Frequency),'\,kHz'];
elseif SymmetricSystem == 0 && HeadLine == 2
    ax.Title.FontSize = FontSizeHeadLine;
    if  Repetitions == 1
        ax.Title.String = ['Wavenumber in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,'] @ ',num2str(Frequency),'\,kHz'];
    elseif Repetitions > 1 
        ax.Title.String = ['Wavenumber in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'}$ @ ',num2str(Frequency),'\,kHz'];
    end
elseif SymmetricSystem == 1 && HeadLine == 2
    ax.Title.FontSize = FontSizeHeadLine;
    if  Repetitions == 1
        ax.Title.String = ['Wavenumber in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{\mathrm s}$ @ ',num2str(Frequency),'\,kHz'];
    elseif Repetitions > 1
        ax.Title.String = ['Wavenumber in ',num2str(PlateThickness*1e3),'\,mm ',char(join(split(Material{1}.Name,'_'),'\_')),' [',LayupString,']$_{',num2str(Repetitions),'\mathrm s}$ @ ',num2str(Frequency),'\,kHz'];
    end
end
z = ax.RTick;
z(1) = '';

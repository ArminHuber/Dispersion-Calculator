...
% =========================================================================
function B = Computer_Anisotropic_B_F(ax,Viscoelastic,FluidLoading,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,FluidDensityThreshold,Material,FrequencyRange,PlateThickness,HigherOrderModes,FLambF,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,c,SuperLayerSize,LayerThicknesses,Repetitions,SymmetricSystem,I,Delta,MissingSamples,BelowCutoffWidth,PhaseVelocityStep,FrequencyOffset)        
if  (~FluidLoading || (FluidLoading &&...
    ((ToggleUpperFluid && UpperFluid.Density < FluidDensityThreshold) ||...
    (ToggleLowerFluid && LowerFluid.Density < FluidDensityThreshold) ||...
...
if  HigherOrderModes && any(H)
    ...
    g = animatedline(ax,'color',[.5 0 1]);
    g(2) = animatedline(ax,'color',[.5 0 1]);
    g(3) = animatedline(ax,'color',[.5 0 1]);
end
...
for i = 1:length(SweepRange)
    ...
    for m = 1:SuperLayerSize
        ...
        A3 = a31(m)+a32(m)*rc2+a33(m)*r2c4+a34(m)*Material{m}.Density^3*SweepRange(i)^6;
        Alphaa = A2/3-A1^2/9;
...
B{1}(:,1) = FrequencyRange(1:height(X));
B{1}(:,2) = FrequencyRange(1:height(X))/1e3;
B{1}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
B{1}(:,4) = fillmissing(X(:,1),'spline')/1e3;
B{1}(:,7) = fillmissing(X(:,2),'spline');
B{1}(:,8) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
B{1}(:,9) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
B{1}(B{1}(:,7) < 0,7) = 0; % negative attenuation is impossible
X = [];
Misses = 0;
...
%     end
    ...
    if  X(i,1) > 0 && FundamentalCutoff(i) == 0
...
if  all(X(:,1) == 0)
    B{2}(1) = FrequencyRange(1);
    B{2}(2) = FrequencyRange(1)/1e3;
    B{2}(3) = FrequencyRange(1)*PlateThickness;
    B{2}(4) = X0(1)/1e3;
    B{2}(7) = 0;
    B{2}(8) = X0(1); % real phase velocity (m/s)
    B{2}(9) = 0; % imaginary phase velocity (m/s)
else
    ...
    X(Misses(1:height(X)) == 1,:) = NaN;
    B{2}(:,1) = FrequencyRange(1:height(X));
    B{2}(:,2) = FrequencyRange(1:height(X))/1e3;
    B{2}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
    B{2}(:,4) = fillmissing(X(:,1),'spline')/1e3;
    B{2}(:,7) = fillmissing(X(:,2),'spline');
    B{2}(:,8) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
    B{2}(:,9) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
    B{2}(B{2}(:,7) < 0,7) = 0; % negative attenuation is impossible
end
X = [];
Misses = 0;
FundamentalCutoff = 0;
...
if  all(X(:,1) == 0)
    B{3}(1) = FrequencyRange(1);
    B{3}(2) = FrequencyRange(1)/1e3;
    B{3}(3) = FrequencyRange(1)*PlateThickness;
    B{3}(4) = X0(2)/1e3;
    B{3}(7) = 0;
    B{3}(8) = X0(2); % real phase velocity (m/s)
    B{3}(9) = 0; % imaginary phase velocity (m/s)
else
    if  any(FundamentalCutoff)
        X(FundamentalCutoff == 1,:) = NaN;
        X(1,:) = [X0(2) 0 X0(2) 0];
    ...
    X(Misses(1:height(X)) == 1,:) = NaN;
    B{3}(:,1) = FrequencyRange(1:height(X));
    B{3}(:,2) = FrequencyRange(1:height(X))/1e3;
    B{3}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
    B{3}(:,4) = fillmissing(X(:,1),'spline')/1e3;
    B{3}(:,7) = fillmissing(X(:,2),'spline');
    B{3}(:,8) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
    B{3}(:,9) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
    B{3}(B{3}(:,7) < 0,7) = 0; % negative attenuation is impossible
end
if  HigherOrderModes && any(H)

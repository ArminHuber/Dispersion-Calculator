...
%
% For more information contact Armin Huber at armin.hubSShearer@dlr.de
% =========================================================================
function AShear = Computer_Anisotropic_AShear_V(ax,Material,Hybrid,FrequencyRange,PlateThickness,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,c,SuperLayerSize,LayerThicknesses,Repetitions,MissingSamples,BelowCutoffWidth)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*MINV>
%#ok<*BDLOG>
...
if  SuperLayerSize == 1
            ...
            if  p > 1
                for j = 1:length(AShear)
                    if  i <= height(AShear{j})
                        Neighbors(j,:) = [AShear{j}(i,7) AShear{j}(i,8)];
                    end
                end
            end
    ...
    % end
                    ...
                    if  any(MIN)
                        if  p > 1 && (numel(find(X(:,1) ~= 0)) <= 3 && i <= height(AShear{p-1}) && AngularFrequency/real(WaveNumber(MIN(1),MIN(2))) < AShear{p-1}(i,4)*1e3) ||...
        ...
        if  MissingModes(p) == 0
            ...
            AShear{p}(:,1) = FrequencyRange(1:height(X));
            AShear{p}(:,2) = FrequencyRange(1:height(X))/1e3;
            AShear{p}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
            AShear{p}(:,4) = fillmissing(X(:,1),'spline')/1e3;
            AShear{p}(:,6) = fillmissing(X(:,2),'spline');
            AShear{p}(:,7) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
            AShear{p}(:,8) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
            AShear{p}(AShear{p}(:,6) < 0,6) = 0; % negative attenuation is impossible
        else
            AShear{p} = AShear{p-1};
        end      
    end

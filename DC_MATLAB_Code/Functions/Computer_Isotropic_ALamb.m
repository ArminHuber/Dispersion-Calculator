...
% =========================================================================
function ALamb = Computer_Isotropic_ALamb(Multithreading,Q1,Q2,ax,Material,FrequencyRange,PhaseVelocitySections,Half,HigherOrderModes,H,FrequencyResolution,PhaseVelocityLimit,PhaseVelocityResolution,LambPhaseVelocitySweepRange1,LambPhaseVelocitySweepRange2,ASteps,AOffset,FrequencySections)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS>
global Stop
        ...
        for j = 1:length(SweepRange)-1
            ...
            for k = 1:Bisections
                ...
                for l = PhaseVelocityIndices(1):PhaseVelocityIndices(end)
                    ...
                    x = sqrt(kL2-k2);
                    y = sqrt(kT2-k2);
                    Y(j,l) = y*tan(y*Half)+(y^2-k2)^2*tan(x*Half)/(4*k2*x);
                end
                if  k == 1
                    Y(j+1,1) = Y(j,3);
                end
...
ALamb{1}(:,1) = FrequencyRange;
ALamb{1}(:,2) = FrequencyRange/1e3;
ALamb{1}(:,3) = FrequencyRange*2*Half;
ALamb{1}(:,4) = X/1e3;
ALamb{1}(:,6) = 0;
if  Multithreading
    send(Q1,ALamb{1})
else
    line(ax,ALamb{1}(:,1),ALamb{1}(:,4),'color','b')
    drawnow limitrate
end
    ...
    for p = 1:length(H)
        ...
        for i = ceil(H(p)/FrequencyResolution)+1:length(FrequencyRange)
            ...
            if  i == ceil(H(p)/FrequencyResolution)+1
                SweepRange = [PhaseVelocityLimit ALamb{p}(i,4)*1e3+1];
            elseif i == ceil(H(p)/FrequencyResolution)+2
                SweepRange = [X(end) ALamb{p}(i,4)*1e3+1];
            else
                ...
                if  SweepRange(end) < ALamb{p}(i,4)*1e3+.1 
                    SweepRange(end) = ALamb{p}(i,4)*1e3+.1;
                end
            end
            for o = 0:PhaseVelocitySections
                if  o > 0
                ...
                for j = 1:length(SweepRange)-1
                    ...
                    for k = 1:Bisections
                        ...
                        if  (j == 1 && abs(real(Y(j,2))) < 1e4) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,1))) ~= sign(real(Y(j,2)))    
                            PhaseVelocity = [PhaseVelocity(1) PhaseVelocity(1)+(PhaseVelocity(2)-PhaseVelocity(1))/2 PhaseVelocity(2)];
                            Y(j,3) = Y(j,2);
                        elseif (j == 1 && abs(real(Y(j,2))) < 1e4) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,2))) ~= sign(real(Y(j,3)))    
                            PhaseVelocity = [PhaseVelocity(2) PhaseVelocity(2)+(PhaseVelocity(3)-PhaseVelocity(2))/2 PhaseVelocity(3)];
                            Y(j,1) = Y(j,2);
                        else
                            PhaseVelocity(2) = 0;
            ...
            if  i <= ceil(H(p)/FrequencyResolution)+2 && PhaseVelocity(2) == 0
                errordlg(['Unable to trace A',num2str(p),'! Increase Phase velocity sections or decrease Frequency step or Phase velocity limit.'],'Error');
                return
        ...
        ALamb{p+1}(:,3) = FrequencyRange(1:length(X))*2*Half;
        ALamb{p+1}(:,4) = X/1e3;
        if  Multithreading
            send(Q1,ALamb{p+1})
        else
            line(ax,ALamb{p+1}((ALamb{p+1}(:,4) ~= 0),1),ALamb{p+1}((ALamb{p+1}(:,4) ~= 0),4),'color','b')
            drawnow limitrate
        end
        [Max,MaxInd] = max(ALamb{p+1}(:,4));
        PhaseVelocityRange = Max*1e3+ASteps:ASteps:PhaseVelocityLimit+ASteps;
        for i = 1:length(PhaseVelocityRange)
            if  i == 1
                SweepRange = [ALamb{p+1}(MaxInd,1)-AOffset/Half/2e3 ALamb{p+1}(MaxInd,1)+4/Half/2e3];
            else
                SweepRange = [X1{p+1}(end)-AOffset/Half/2e3 X1{p+1}(end)+4/Half/2e3];
            end

...
% =========================================================================
function SShear = Computer_Isotropic_SShear_V(Material,FrequencyRange,Thickness,HigherOrderModes,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,SSteps,SOffset)
%#ok<*AGROW>
...
if  HigherOrderModes && any(H)
    for p = 1:length(H)
        ...
        WaveNumber = sqrt((AngularFrequency/Material.TransverseVelocity_complex).^2-4*(p*pi/Thickness)^2);
        ...
        if  max(SShear{p+1}(:,4))*1e3 > PhaseVelocityLimit
            X1{p+1}(1,1) = 0;
        else
            [~,MaxInd] = max(SShear{p+1}(:,4));
            AngularFrequency = 2*pi*SShear{p+1}(MaxInd,1)*1e3;
            WaveNumber = AngularFrequency/(SShear{p+1}(MaxInd,4)*1e3)+1i*SShear{p+1}(MaxInd,6);
            PhaseVelocityComplex = AngularFrequency/WaveNumber;
            PhaseVelocityRange = real(PhaseVelocityComplex)+SSteps:SSteps:PhaseVelocityLimit+SSteps;
            for i = 1:length(PhaseVelocityRange)
                ...
                for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeFrq X SweepRangeImag)
                    if  i == 1
                        SweepRangeFrq = [SShear{p+1}(MaxInd,1)-SOffset/Thickness/1e3 SShear{p+1}(MaxInd,1)+4/Thickness/1e3];
                        SweepRangeImag = [SearchWidthImag(1)*imag(PhaseVelocityComplex) SearchWidthImag(2)*imag(PhaseVelocityComplex)];
                    else
                        SweepRangeFrq = [X1{p+1}(end-1,1)-SOffset/Thickness/1e3 X1{p+1}(end-1,1)+4/Thickness/1e3];
                            ...
                            if  ~isempty(b1) % one or multiple minima are found
                                if  length(b1) == 1
                                    ...
                                    for l = 1:length(b1) % calculate which minimum lies closest to last solution
                                        ...
                                        if  i == 1
                                            delta(l) = abs(frq(l)-SShear{p+1}(MaxInd,1));
                                        else

...
%
% For more information contact Armin Huber at armin.huber@dlr.deA{130}
% =========================================================================
...
A{14} = A{10}.*((.87+1.12*A{4})./(1+A{4})); % Rayleigh wave velocity (m/s)
...
A{15} = Lambda_real+1i*Lambda_imag; % complex Lame's first parameter, Lambda (Pa)
A{16} = Mu_real+1i*Mu_imag; % complex Lame's second parameter, Mu (Pa)
A{17} = A{16}.*((3*A{15}+2*A{16})./(A{15}+A{16})); % complex Young's modulus (GPa)
A{18} = A{15}./(2*(A{15}+A{16})); % complex Poisson's ratio
A{19}{1,1} = A{15}+2*A{16}; % complex stiffness component C11 (Pa) 
A{19}{1,2} = A{15}; % C12 (Pa)
A{19}{1,3} = A{15}; % C13 (Pa)
A{19}{2,2} = A{15}+2*A{16}; % C22 (Pa)
A{19}{2,3} = A{15}; % C23 (Pa)
A{19}{3,3} = A{15}+2*A{16}; % C33 (Pa)
A{19}{4,4} = A{16}; % C44 (Pa)
A{19}{5,5} = A{16}; % C55 (Pa)
A{19}{6,6} = A{16}; % C66 (Pa)
...
for i = 1:size(A{1},1) % enter all materials contained in A
    ...
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'YoungsModulus_complex',A{17}(i));
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'PoissonsNumber',A{4}(i));
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'PoissonsNumber_complex',A{18}(i));
    ...
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'Lambda_complex',A{15}(i));
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'Mu',A{8}(i));
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'Mu_complex',A{16}(i));
    ...
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'RayleighVelocity',A{14}(i));
    Materials.Isotropic = setfield(Materials.Isotropic,{1,1},cell2mat(A{1}(i)),{1,1},'C',[A{19}{1,1}(i) A{19}{1,2}(i) A{19}{1,3}(i) 0 0 0;0 A{19}{2,2}(i) A{19}{2,3}(i) 0 0 0;0 0 A{19}{3,3}(i) 0 0 0;0 0 0 A{19}{4,4}(i) 0 0;0 0 0 0 A{19}{5,5}(i) 0;0 0 0 0 0 A{19}{6,6}(i)]);    
end

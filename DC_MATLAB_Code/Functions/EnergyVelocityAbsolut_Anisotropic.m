...
if  Symmetric
    if  SymmetricModes && ~isempty(S{1})
        S{1}(:,5) = sqrt(S{1}(:,5).^2+S{1}(:,6).^2); % calculate energy velocity absolute
    ...
    if  ScholteModes && ~isempty(BScholte{1})
        for i = 1:length(BScholte)
            BScholte{i}(:,5) = sqrt(BScholte{i}(:,5).^2+BScholte{i}(:,6).^2);
...
function output_txt = Cursor(~,event_obj)
    ...
    if  XAxisMode == 1 && length(event_obj.Target.XData) ~= 2
        output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} kHz'] ['$|\vec{c}_{\mathrm e}|$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
    elseif XAxisMode == 2 && length(event_obj.Target.XData) ~= 2
        output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} MHz'] ['$|\vec{c}_{\mathrm e}|$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
    elseif XAxisMode == 3 && length(event_obj.Target.XData) ~= 2
        output_txt = {['\textbf{',ModeName,'}'] ['$f\cdot d$: \textbf{',num2str(event_obj.Position(1),6),'} MHz$\cdot$mm'] ['$|\vec{c}_{\mathrm e}|$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};

...
% =========================================================================
function EnergyVelocity2_Anisotropic(Hybrid,Crop,Phi,LayupString,FluidLoading,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,BulkVelocities,PNGresolution,SColor,AColor,BColor,A,AScholte,AntisymmetricModes,B,BScholte,BoxLineWidth,Directory,Export,FontSizeAxes,FontSizeAxesLabels,FontSizeHeadLine,FontSizeModeLabels,FileName,HeadLine,HigherOrderModes,LineWidth,Material,ModeLabels,ModeLabel1X,ModeLabel2X,ModeLabel3X,PDF,PlateThickness,PNG,PropagationAngle,S,SScholte,ScholteModes,Repetitions,SuperLayerSize,SymmetricModes,SymmetricSystem,Symmetric,XAxis,XAxisMode,YAxis,Decoupled)        
%#ok<*FXUP>
%#ok<*AGROW>
%#ok<*CHAIN>
...
if  Symmetric
    if  SymmetricModes && ~isempty(S{1})
        s = plot(S{1}(:,XAxisMode),S{1}(:,6),'LineWidth',LineWidth,'Color',SColor);
        if  ModeLabels
            z = find(abs(S{1}(:,1)-(ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(S{1}(:,1)-(ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1)),S{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,S{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,S{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        s(2) = plot(S{2}(:,XAxisMode),S{2}(:,6),'LineWidth',LineWidth,'Color',SColor);
        if  ModeLabels
            z = find(abs(S{2}(:,1)-(ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(S{2}(:,1)-(ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1)),S{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,S{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,S{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'S$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        if  HigherOrderModes
            for i = 3:size(S,2)
                s(i) = plot(S{i}(~S{i}(:,6) == 0,XAxisMode),S{i}(~S{i}(:,6) == 0,6),'LineWidth',LineWidth,'Color',SColor);
            end
        end
    end
    if  AntisymmetricModes && ~isempty(A{1})
        a = plot(A{1}(:,XAxisMode),A{1}(:,6),'LineWidth',LineWidth,'Color',AColor);
        if  ModeLabels
            z = find(abs(A{1}(:,1)-(ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(A{1}(:,1)-(ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1)),A{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'A$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,A{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'A$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,A{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'A$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        if  HigherOrderModes
            for i = 2:size(A,2)
                a(i) = plot(A{i}(~A{i}(:,6) == 0,XAxisMode),A{i}(~A{i}(:,6) == 0,6),'LineWidth',LineWidth,'Color',AColor);
            end
        end
    end
    ...
    if  ~isempty(B{1})
        b = plot(B{1}(:,XAxisMode),B{1}(:,6),'LineWidth',LineWidth,'Color',BColor);
        if  ModeLabels
            z = find(abs(B{1}(:,1)-(ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(B{1}(:,1)-(ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1)),B{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,B{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel3X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,B{1}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_0$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        b(2) = plot(B{2}(:,XAxisMode),B{2}(:,6),'LineWidth',LineWidth,'Color',BColor);
        if  ModeLabels
            z = find(abs(B{2}(:,1)-(ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(B{2}(:,1)-(ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1)),B{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,B{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel2X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,B{2}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_1$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        b(3) = plot(B{3}(:,XAxisMode),B{3}(:,6),'LineWidth',LineWidth,'Color',BColor);
        if  ModeLabels
            z = find(abs(B{3}(:,1)-(ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))) == min(abs(B{3}(:,1)-(ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1)))));
            if  XAxisMode == 1
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1)),B{3}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_2$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 2
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))/1e3,B{3}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_2$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            elseif XAxisMode == 3
                text((ModeLabel1X*(XAxis(2)-XAxis(1))+XAxis(1))*PlateThickness,B{3}(z(1),6)+(YAxis(2)-YAxis(1))/30,'B$_2$','FontSize',FontSizeModeLabels,'Interpreter','latex');
            end
        end
        if  HigherOrderModes
            for i = 4:size(B,2)
                b(i) = plot(B{i}(~B{i}(:,6) == 0,XAxisMode),B{i}(~B{i}(:,6) == 0,6),'LineWidth',LineWidth,'Color',BColor);
            end
        end
    end
    ...
    try
        if  PDF
            if  ~Crop
                ...
                print(f,fullfile(Directory,[FileName,'_ce2']),'-dpdf')
            else
                exportgraphics(f,fullfile(Directory,[FileName,'_ce2.pdf']))
            end
        end
        if  PNG
            if  ~Crop
                print(f,fullfile(Directory,[FileName,'_ce2']),'-dpng',['-r',num2str(PNGresolution)])
            else
...
function output_txt = Cursor(~,event_obj)
    if  Symmetric
        if  ~isempty(S{1}) && all(event_obj.Target.Color == SColor)
            for i = 1:length(S)
                if  i == 1 && event_obj.Target.YData(1) == S{1}(1,6)
                    ModeName = 'S$_0$';
                    break
                elseif i == 2 && event_obj.Target.YData(1) == S{2}(1,6)
                    ModeName = 'S$_1$';
                    break
        ...
        if  ~isempty(A{1}) && all(event_obj.Target.Color == AColor)
            for i = 1:length(A)
                if  i == 1 && event_obj.Target.YData(1) == A{1}(1,6)
                    ModeName = 'A$_0$';
                    break
        ...
        if  ~isempty(B{1})
            for i = 1:length(B)
                if  i == 1 && event_obj.Target.YData(1) == B{1}(1,6)
                    ModeName = 'B$_0$';
                    break 
                elseif i == 2 && event_obj.Target.YData(1) == B{2}(1,6)
                    ModeName = 'B$_1$';
                    break
                elseif i == 3 && event_obj.Target.YData(1) == B{3}(1,6)
                    ModeName = 'B$_2$';
                    break
    ...
    elseif XAxisMode == 3 && length(event_obj.Target.XData) ~= 2
        output_txt = {['\textbf{',ModeName,'}'] ['$f\cdot d$: \textbf{',num2str(event_obj.Position(1),6),'} MHz$\cdot$mm'] ['$c_{\mathrm{e}2}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};

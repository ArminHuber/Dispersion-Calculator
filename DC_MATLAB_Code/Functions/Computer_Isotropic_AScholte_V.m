...
% =========================================================================
function AScholte = Computer_Isotropic_AScholte_V(Multithreading,Q,ax,Fluid,Material,FrequencyRange,Half,HigherOrderModes,FAScholte,H,FrequencyResolution,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,MissingSamples,BelowCutoffWidth)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS>
global Stop
...
for i = 1:length(FrequencyRange)
    ...
    for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeReal X SweepRangeImag)
        if  i < 4
            SweepRangeReal = [2*FAScholte(1) .95*FAScholte(1)];
        else
...
AScholte{1}(:,3) = FrequencyRange(1:height(X))*2*Half;
...
% end
        ...
        if  MissingModes(p+1) == 0
            ...
            AScholte{p+1}(:,3) = FrequencyRange(1:height(X))*2*Half;

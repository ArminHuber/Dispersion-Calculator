...
% =========================================================================
function SLamb = Computer_Isotropic_SLamb(Multithreading,Q1,Q2,ax,Material,FrequencyRange,PhaseVelocitySections,Half,HigherOrderModes,H,FrequencyResolution,PhaseVelocityLimit,PhaseVelocityResolution,LambPhaseVelocitySweepRange1,LambPhaseVelocitySweepRange2,SSteps,SOffset,FrequencySections)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS>
global Stop
        ...
        for j = 1:length(SweepRange)-1
            ...
            for k = 1:Bisections
                ...
                for l = PhaseVelocityIndices(1):PhaseVelocityIndices(end)
                    ...
                    x = sqrt(kL2-k2);
                    y = sqrt(kT2-k2);
                    Y(j,l) = tan(y*Half)/y+4*k2*x*tan(x*Half)/(y^2-k2)^2;
                end
                if  k == 1
                    Y(j+1,1) = Y(j,3);
                end
                if  (j == 1 && abs(real(Y(j,2))) < 1e1) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,1))) ~= sign(real(Y(j,2)))    
                    PhaseVelocity = [PhaseVelocity(1) PhaseVelocity(1)+(PhaseVelocity(2)-PhaseVelocity(1))/2 PhaseVelocity(2)];
                    Y(j,3) = Y(j,2);
                elseif (j == 1 && abs(real(Y(j,2))) < 1e1) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,2))) ~= sign(real(Y(j,3)))    
...
if  HigherOrderModes && any(H)
    for p = 1:length(H)
            ...
            for o = 0:PhaseVelocitySections
                ...
                for j = 1:length(SweepRange)-1
                    ...
                    for k = 1:Bisections
                        ...
                        if  (j == 1 && abs(real(Y(j,2))) < 1e-1) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,1))) ~= sign(real(Y(j,2)))    
                            PhaseVelocity = [PhaseVelocity(1) PhaseVelocity(1)+(PhaseVelocity(2)-PhaseVelocity(1))/2 PhaseVelocity(2)];
                            Y(j,3) = Y(j,2);
                        elseif (j == 1 && abs(real(Y(j,2))) < 1e-1) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,2))) ~= sign(real(Y(j,3)))    
                            PhaseVelocity = [PhaseVelocity(2) PhaseVelocity(2)+(PhaseVelocity(3)-PhaseVelocity(2))/2 PhaseVelocity(3)];
                            Y(j,1) = Y(j,2);
                        else
                            PhaseVelocity(2) = 0;
        ...
        PhaseVelocityRange = Max*1e3+SSteps:SSteps:PhaseVelocityLimit+SSteps;
        for i = 1:length(PhaseVelocityRange)
            if  i == 1
                if  p == 1
                    SweepRange = [SLamb{p+1}(MaxInd,1)-2*SOffset/Half/2e3 SLamb{p+1}(MaxInd,1)+10/Half/2e3];
                else
                    SweepRange = [SLamb{p+1}(MaxInd,1)-SOffset/Half/2e3 SLamb{p+1}(MaxInd,1)+10/Half/2e3];
                end
            else
                if  p == 1
                    SweepRange = [X1{p+1}(end)-2*SOffset/Half/2e3 X1{p+1}(end)+10/Half/2e3];
                else
                    SweepRange = [X1{p+1}(end)-SOffset/Half/2e3 X1{p+1}(end)+10/Half/2e3];
                end
            end

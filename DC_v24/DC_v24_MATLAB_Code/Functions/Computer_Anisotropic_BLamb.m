...
for p = 1:2
    ...
    for i = 1:length(FrequencyRange)
        ...
        if  p == 1
            ...
            elseif i > 1 && i < 6
                SweepRange = X(i-1)+50:-5:X(i-1)-50;
            else
...
% end
        ...
        if  ~MissingModes(p+2)
            X(Misses(1:length(X)) == 1) = NaN;
            BLamb{p+2}(:,1) = FrequencyRange(1:length(X));
            BLamb{p+2}(:,2) = FrequencyRange(1:length(X))/1e3;
            BLamb{p+2}(:,3) = FrequencyRange(1:length(X))*PlateThickness;
            BLamb{p+2}(:,4) = fillmissing(X,'spline')/1e3;
        else
            BLamb{p+2} = BLamb{p};
            X1{p+2}(1) = 0;
            continue
        end
        [Max,MaxInd] = max(BLamb{p+2}(:,4));
        ...
        for i = 1:length(PhaseVelocityRange)
            ...
            if  i == 1
                if  p == 1
                    SweepRange = [BLamb{p+2}(MaxInd,1)-2*FrequencyOffset/PlateThickness/1e3 BLamb{p+2}(MaxInd,1)+2/PlateThickness/1e3];
                else
                    SweepRange = [BLamb{p+2}(MaxInd,1)-FrequencyOffset/PlateThickness/1e3 BLamb{p+2}(MaxInd,1)+2/PlateThickness/1e3];
                end
            else
                if  p == 1

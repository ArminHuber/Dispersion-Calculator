...
% =========================================================================
function ModeShapeLines_Isotropic(FunctionMode,MakeFigure,ExportData,XSLX,TXT,MAT,Crop,Plot,PNGresolution,Material,Viscoelastic,FluidLoading,Fluid,Color1,Color2,Color3,Color4,Color5,Color6,ALamb,AShear,AScholte,BoxLineWidth,Directory,FileName,Export,FontSizeAxes,FontSizeAxesLabels,FontSizeHeadLine,FontSizeLegend,Frequency,HeadLine,LegendLocation,LineWidth,Mode,PDF,PNG,Thickness,SLamb,SShear,SScholte,SamplesX3,ShowHalfSpaces,HalfSpaces,Phase)
%#ok<*AGROW>
%#ok<*FNDSB>
...
if  ~contains(Mode,'SH')
    ...
    if  FluidLoading && ShowHalfSpaces
        ...
        EFluid = exp(1i*k3Fluid*x3Fluid);
        E1Fluid = 1i*k; % epsilon11
        E3Fluid = 1i*k3Fluid*WFluid; % epsilon33
        uFluid0(:,1) = U(4)*EFluid;
        uFluid0(:,3) = -WFluid*U(4)*EFluid;
        vFluid0 = -1i*AngularFrequency*uFluid0;
        sigmaFluid0(:,1) = DFluid*U(4)*EFluid; % sigma11
        sigmaFluid0(:,2) = sigmaFluid0(:,1); % sigma22
        sigmaFluid0(:,3) = sigmaFluid0(:,1); % sigma33
        sigmaFluid0(:,6) = 0;
        epsilonFluid0(:,1) = E1Fluid*U(4)*EFluid; % epsilon11
        epsilonFluid0(:,3) = E3Fluid*U(4)*EFluid; % epsilon33
        epsilonFluid0(:,6) = 0;
        StrainEnergyDensityFluid0 = .5*(imag(epsilonFluid0(:,1)).*imag(sigmaFluid0(:,1))+real(epsilonFluid0(:,1)).*real(sigmaFluid0(:,1))+imag(epsilonFluid0(:,3)).*imag(sigmaFluid0(:,3))+real(epsilonFluid0(:,3)).*real(sigmaFluid0(:,3)));
        KineticEnergyDensityFluid0 = .5*(Fluid.Density*(abs(vFluid0(:,1)).^2+abs(vFluid0(:,3)).^2));
        PowerFlowDensityFluid0(:,1) = -.5*real(sigmaFluid0(:,1).*conj(vFluid0(:,1)));
        PowerFlowDensityFluid0(:,3) = -.5*real(sigmaFluid0(:,3).*conj(vFluid0(:,3)));
        uFluid1(:,1) = U(5)*EFluid;
        uFluid1(:,3) = WFluid*U(5)*EFluid;
        vFluid1 = -1i*AngularFrequency*uFluid1;
        sigmaFluid1(:,1) = DFluid*U(5)*EFluid; % sigma11
        sigmaFluid1(:,2) = sigmaFluid1(:,1); % sigma22
        sigmaFluid1(:,3) = sigmaFluid1(:,1); % sigma33
        sigmaFluid1(:,6) = 0;
        epsilonFluid1(:,1) = E1Fluid*U(5)*EFluid; % epsilon11
        epsilonFluid1(:,3) = E3Fluid*U(5)*EFluid; % epsilon33
        epsilonFluid1(:,6) = 0;
        StrainEnergyDensityFluid1 = .5*(imag(epsilonFluid1(:,1)).*imag(sigmaFluid1(:,1))+real(epsilonFluid1(:,1)).*real(sigmaFluid1(:,1))+imag(epsilonFluid1(:,3)).*imag(sigmaFluid1(:,3))+real(epsilonFluid1(:,3)).*real(sigmaFluid1(:,3)));
        KineticEnergyDensityFluid1 = .5*(Fluid.Density*(abs(vFluid1(:,1)).^2+abs(vFluid1(:,3)).^2));
        PowerFlowDensityFluid1(:,1) = -.5*real(sigmaFluid1(:,1).*conj(vFluid1(:,1)));
        PowerFlowDensityFluid1(:,3) = -.5*real(sigmaFluid1(:,3).*conj(vFluid1(:,3)));
    end
elseif contains(Mode,'SH')
    ...
    if  FluidLoading && ShowHalfSpaces
        x3Fluid = 0:Thickness/SamplesX3:HalfSpaces*Thickness;
        uFluid0(length(x3Fluid),3) = 0;
        sigmaFluid0(length(x3Fluid),6) = 0;
        epsilonFluid0(length(x3Fluid),6) = 0;
        StrainEnergyDensityFluid0(length(x3Fluid),1) = 0;
        KineticEnergyDensityFluid0(length(x3Fluid),1) = 0;
        PowerFlowDensityFluid0(length(x3Fluid),3) = 0;
        uFluid1(length(x3Fluid),3) = 0;
        sigmaFluid1(length(x3Fluid),6) = 0;
        epsilonFluid1(length(x3Fluid),6) = 0;
        StrainEnergyDensityFluid1(length(x3Fluid),1) = 0;
        KineticEnergyDensityFluid1(length(x3Fluid),1) = 0;
        PowerFlowDensityFluid1(length(x3Fluid),3) = 0;
        if  Phase
            uPhase = vertcat(zeros(length(x3Fluid),3),rad2deg(angle(u)),zeros(length(x3Fluid),3));
            sigmaPhase = vertcat(zeros(length(x3Fluid),6),rad2deg(angle(sigma)),zeros(length(x3Fluid),6));
            epsilonPhase = vertcat(zeros(length(x3Fluid),6),rad2deg(angle(epsilon)),zeros(length(x3Fluid),6));
        end
    else
        if  Phase
...
if  FluidLoading && ShowHalfSpaces
    ...
    sigma = vertcat(flipud(sigmaFluid0),sigma,sigmaFluid1);
    epsilon = vertcat(flipud(epsilonFluid0),epsilon,epsilonFluid1);
    StrainEnergyDensity = vertcat(flipud(StrainEnergyDensityFluid0),StrainEnergyDensity,StrainEnergyDensityFluid1);
    KineticEnergyDensity = vertcat(flipud(KineticEnergyDensityFluid0),KineticEnergyDensity,KineticEnergyDensityFluid1);
    PowerFlowDensity = vertcat(flipud(PowerFlowDensityFluid0),PowerFlowDensity,PowerFlowDensityFluid1);   
end
if  ~contains(Mode,'SH')
    if  Phase
        if  FluidLoading && ShowHalfSpaces
...
if  MakeFigure
    ...
    elseif FunctionMode == 4
        if  strcmp(LegendLocation,'in')
            f = figure('Name','Energy density','Toolbar','none','Units','normalized','OuterPosition',[0 0 .72 1],'color','w');
        else
...
function output_txt = Cursor(~,event_obj)
    if  length(event_obj.Target.XData) == 4
        output_txt = char(join(split(Fluid.Name,'_'),'\_'));
    else
        if  FunctionMode == 1 

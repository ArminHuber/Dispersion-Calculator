...
% =========================================================================
function GroupVelocity_BulkWave2D(Crop,Material,Plane,Phi,ThetaStep,Export,PDF,PNG,FileName,Directory,HeadLine,LineWidth,FontSizeHeadLine,FontSizeAxesLabels,FontSizeAxes,FontSizeModeLabels,PNGresolution)
%#ok<*AGROW>
...
elseif HeadLine == 1
    ...
    if  Plane == 13
        ax.Title.String = ['Bulk wave group velocity @ $\phi_\mathrm r$ = ',num2str(Phi),'\,$^{\circ}$ in ',char(join(split(Material.Name,'_'),'\_'))];
    elseif Plane == 12
        ax.Title.String = ['Bulk wave group velocity @ $\theta_\mathrm r$ = ',num2str(Phi),'\,$^{\circ}$ in ',char(join(split(Material.Name,'_'),'\_'))];
    end
end
z = ax.RTick;
z(1) = '';

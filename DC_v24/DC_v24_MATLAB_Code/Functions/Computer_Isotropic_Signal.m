...
% =========================================================================
function [ExcitationMagnitude,ExcitationSignal,ExcitationSpectrum,FourierTransformLength,Frequency,FrequencyLimitLow,FrequencyLimitHigh,FrequencyRange,PhaseVelocityALamb,PhaseVelocityAShear,PhaseVelocitySLamb,PhaseVelocitySShear,PlotXAxis,Gate,XAxis,uSum,uSumALamb,uSumAShear,uSumSLamb,uSumSShear] = Computer_Isotropic_Signal(FluidLoading,Fluid,DisplacementComponent,ALamb,ALambModes,AShear,AShearModes,Cycles,Distance,Frequency,FrequencyResolution,Material,Mode,MultiMode,SamplesPerCycle,SLamb,SLambModes,SpectrumThreshold,SShear,SShearModes,Thickness,TimeLimitFactor,Window,OutputWindowUI3)
%#ok<*AGROW>
...
if  ~MultiMode
    ...
    if  ~contains(Mode,'SH') % for Lamb waves
        if  Mode(1) == 'S' % for symmetric Lamb waves
            q1 = find(abs(ExcitationSpectrum(1,1)-SLamb{p}(:,1)) == min(abs(ExcitationSpectrum(1,1)-SLamb{p}(:,1)))); % get the index of the starting frequency of the calculated frequency spectrum
            z = find(ischange(SLamb{p}(:,1),'linear'),1,'last'); % find the index of the first frequency from a phase velocity sweep where we have equidistant frequencies; this is to exclude the high phase velocity part of higher order modes obtained by frequency sweeps where we don't have equidistant frequencies; the energy velocity of this range is set to zero and can be identified by that way
            if  isempty(z)
                z = 1;
            end
            ...
            z1 = find(abs(ExcitationSpectrum(1,:)-SLamb{p}(q1,1)) == min(abs(ExcitationSpectrum(1,:)-SLamb{p}(q1,1)))); % find the index in the calculated frequency spectrum corresponding the starting frequency (q1) determined above; we will shorten the frequency spectrum for the superposition of the wave components below
            q2 = find(abs(ExcitationSpectrum(1,end)-SLamb{p}(:,1)) == min(abs(ExcitationSpectrum(1,end)-SLamb{p}(:,1)))); % get the index of the ending frequency of the calculated frequency spectrum
            z2 = find(abs(ExcitationSpectrum(1,:)-SLamb{p}(q2,1)) == min(abs(ExcitationSpectrum(1,:)-SLamb{p}(q2,1)))); % find the index corresponding the ending frequency (q2) determined above
    ...
    if  CoherenceTime < TimeLimit % if the coherence time is smaller than the calculated time range, we have to expect unwanted twins of the wave packet
    ...
    [uSum,~,ExcitationSpectrumRange] = Computer_Isotropic_Signal_Core(FluidLoading,Fluid,DisplacementComponent,CoherenceTime,0,Distance/1e3,ExcitationSpectrum,SamplesPerCycle,Cycles,FrequencyResolution,FrequencyResolution2,0,Material,ModeType,0,PhaseVelocity,Attenuation,Thickness,Time,TimeLimit,z1,z2);
    if  isempty(uSum{1})
        return
    end
    ...
    EnergyVelocity(3,1) = min(EnergyVelocitySShear);
    EnergyVelocity(3,2) = max(EnergyVelocitySShear);
    ...
    EnergyVelocity(4,1) = min(EnergyVelocityAShear);
    EnergyVelocity(4,2) = max(EnergyVelocityAShear);
    ...
    ModeTotal = length(EnergyVelocityALamb(~isnan(EnergyVelocityALamb)))+length(EnergyVelocitySLamb(~isnan(EnergyVelocitySLamb)))+length(EnergyVelocityAShear(~isnan(EnergyVelocityAShear)))+length(EnergyVelocitySShear(~isnan(EnergyVelocitySShear)));
    tic
    h1 = waitbar(0,sprintf('0 of %d (0 %%)',ModeTotal),'Name','Calculating mode...'); % generate waitbar
    Counter = 0;
    if  any(cellfun(@any,PhaseVelocitySLamb))
        [uSumSLamb,Counter,ExcitationSpectrumRangeSLamb] = Computer_Isotropic_Signal_Core(FluidLoading,Fluid,DisplacementComponent,CoherenceTime,Counter,Distance/1e3,ExcitationSpectrum,SamplesPerCycle,Cycles,FrequencyResolution,FrequencyResolution2,h1,Material,'SLamb',ModeTotal,PhaseVelocitySLamb,AttenuationSLamb,Thickness,Time,TimeLimit,z1SLamb,z2SLamb);

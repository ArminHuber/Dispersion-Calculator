...
% =========================================================================
function BLamb = Computer_Anisotropic_BLamb_F(ax,Viscoelastic,FluidLoading,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,FluidDensityThreshold,Material,Hybrid,FrequencyRange,PlateThickness,HigherOrderModes,FLambF,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,c,SuperLayerSize,LayerThicknesses,Repetitions,SymmetricSystem,I1,MissingSamples,BelowCutoffWidth,PhaseVelocityStep,FrequencyOffset)        
    ...
    for o = 0:6
        ...
        for j = 1:length(SweepRange)-1
            ...
            for k = 1:24
                ...
                if  (j == 1 && abs(real(Y(j,2))) < 1e40) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,1))) ~= sign(real(Y(j,2)))
                    PhaseVelocity = [PhaseVelocity(1) PhaseVelocity(1)+(PhaseVelocity(2)-PhaseVelocity(1))/2 PhaseVelocity(2)];
                    Y(j,3) = Y(j,2);
                elseif (j == 1 && abs(real(Y(j,2))) < 1e40) | (j > 1 && abs(real(Y(j,2))) < abs(real(Y(j-1,2)))) && sign(real(Y(j,2))) ~= sign(real(Y(j,3)))   
...
if  HigherOrderModes && any(H)
    ...
    for p = 1:length(H)
        ...
        [BLamb,X1,MissingModes] = Computer_Anisotropic_BLamb_F_HigherModes(FluidLoading,FluidVelocity,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,Material,FrequencyRange,PlateThickness,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,c,SuperLayerSize,LayerThicknesses,Repetitions,SymmetricSystem,I1,MissingSamples,BelowCutoffWidth,PhaseVelocityStep,FrequencyOffset,BLamb,X1,A1,MissingModes,X0,T,p,g,g1,a21,a22,a31,a32);   
        clear Computer_Anisotropic_BLamb_F_HigherModes
...
%                                             A3 = a31(m)*WaveNumber4(j,l)+a32(m)*rw2*WaveNumber2(j,l)+Material{m}.Density^2*AngularFrequency(j)^4;
%                                             k3(1) = sqrt((-A2+sqrt(A2^2-2*A1(m)*A3))/A1(m));
%                                             k3(2) = sqrt((-A2-sqrt(A2^2-2*A1(m)*A3))/A1(m));

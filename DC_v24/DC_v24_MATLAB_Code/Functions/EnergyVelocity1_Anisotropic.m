...
% =========================================================================
function EnergyVelocity1_Anisotropic(Hybrid,Crop,Phi,LayupString,FluidLoading,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,BulkVelocities,PNGresolution,SColor,AColor,BColor,A,ALamb,AntisymmetricModes,AShear,AScholte,B,BLamb,BoxLineWidth,BShear,BScholte,Directory,Export,FontSizeAxes,FontSizeAxesLabels,FontSizeHeadLine,FontSizeModeLabels,FileName,HeadLine,HigherOrderModes,LambModes,LineWidth,Material,ModeLabels,ModeLabel1X,ModeLabel2X,ModeLabel3X,PDF,PlateThickness,PNG,PropagationAngle,S,ShearHorizontalModes,ScholteModes,SLamb,SShear,SScholte,Repetitions,SuperLayerSize,SymmetricModes,SymmetricSystem,Symmetric,XAxis,XAxisMode,YAxis,Decoupled)
%#ok<*FXUP>
%#ok<*AGROW>
%#ok<*CHAIN>
if  ~Decoupled
    f = figure('Name','Energy velocity ce1','Toolbar','none','Units','normalized','OuterPosition',[0 0 1 1],'color','w');
else
    f = figure('Name','Energy velocity','Toolbar','none','Units','normalized','OuterPosition',[0 0 1 1],'color','w');
end
...
if  ~Decoupled
    ax.YLabel.String = 'Energy velocity $c_{\mathrm e1}$ (m/ms)';
else
    ...
    try
        if  PDF
            if  ~Crop
                ...
                if  ~Decoupled
                    print(f,fullfile(Directory,[FileName,'_ce1']),'-dpdf')
                else
                    print(f,fullfile(Directory,FileName),'-dpdf')
                end
            else
                if  ~Decoupled
                    exportgraphics(f,fullfile(Directory,[FileName,'_ce1.pdf']))
                else
        ...
        if  PNG
            if  ~Crop
                if  ~Decoupled
                    print(f,fullfile(Directory,[FileName,'_ce1']),'-dpng',['-r',num2str(PNGresolution)])
                else
...
function output_txt = Cursor(~,event_obj)
    ...
    if  XAxisMode == 1 && length(event_obj.Target.XData) ~= 2
        if  ~Decoupled
            output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} kHz'] ['$c_{\mathrm{e}1}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        else
            output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} kHz'] ['$c_{\mathrm{e}}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        end
    elseif XAxisMode == 2 && length(event_obj.Target.XData) ~= 2
        if  ~Decoupled
            output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} MHz'] ['$c_{\mathrm{e}1}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        else
            output_txt = {['\textbf{',ModeName,'}'] ['$f$: \textbf{',num2str(event_obj.Position(1),6),'} MHz'] ['$c_{\mathrm{e}}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        end
    elseif XAxisMode == 3 && length(event_obj.Target.XData) ~= 2
        if  ~Decoupled
            output_txt = {['\textbf{',ModeName,'}'] ['$f\cdot d$: \textbf{',num2str(event_obj.Position(1),6),'} MHz$\cdot$mm'] ['$c_{\mathrm{e}1}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        else
            output_txt = {['\textbf{',ModeName,'}'] ['$f\cdot d$: \textbf{',num2str(event_obj.Position(1),6),'} MHz$\cdot$mm'] ['$c_{\mathrm{e}}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        end
    elseif all(event_obj.Target.Color == [0 1 1]) && event_obj.Target.YData(1) == S_fast/1e3
        if  ~Decoupled
            ...
            output_txt = {'\textbf{Fast shear phase velocity}' ['$v_{\mathrm S_{\mathrm{fast}}}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        end
    elseif all(event_obj.Target.Color == [0 1 1]) && event_obj.Target.YData(1) == S_slow/1e3
        if  Decoupled && PropagationAngle == 0 && strcmp(Material{1}.Class,'Transversely isotropic')
            output_txt = {'\textbf{Fast \& slow shear phase velocity}' ['$v_{\mathrm S}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        elseif Decoupled && ((PropagationAngle == 90 && strcmp(Material{1}.Class,'Transversely isotropic')) || strcmp(Material{1}.Class,'Orthotropic'))
            output_txt = {'\textbf{Slow shear phase velocity}' ['$v_{\mathrm S_{\mathrm{slow}}}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        else
    ...
    elseif all(event_obj.Target.Color == [0 1 1]) && event_obj.Target.YData(1) == L/1e3
        if  ~Decoupled
            ...
            output_txt = {'\textbf{Longitudinal phase velocity}' ['$v_{\mathrm L}$: \textbf{',num2str(event_obj.Position(2),6),'} m/ms']};
        end

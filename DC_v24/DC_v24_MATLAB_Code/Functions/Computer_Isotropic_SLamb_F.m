...
% =========================================================================
function SLamb = Computer_Isotropic_SLamb_F(Multithreading,Q1,Q2,ax,Viscoelastic,FluidLoading,Fluid,Material,FrequencyRange,Half,HigherOrderModes,H,FrequencyResolution,PhaseVelocityLimit,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,SSteps,SOffset,MissingSamples,BelowCutoffWidth)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS> 
global Stop 
...
for i = 1:length(FrequencyRange)
    ...
    for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeReal X SweepRangeImag)
        if  i < 4
            SweepRangeReal = [Material.PlateVelocity+2*Fluid.Density/1e3 Material.PlateVelocity-2*Fluid.Density/1e3];
        else
...
% end
    ...
    if  i > 2 && X(end,3) < Fluid.Velocity && abs(X(end,4)) < Resolution % avoid jumping to Scholte modes
        X(end,:) = [];
        break
    end
    if  Multithreading
...
% String = ['f = ',num2str(FrequencyRangeF(i)),' i = ',num2str(i),' q = ',num2str(q),' o = ',num2str(o),' k = ',num2str(k)];
% if  Misses(i) == 1 
%     String = append(String,' Miss');
% end
% disp(String)
% if  Misses(i) == 1
%     disp(['f = ',num2str(FrequencyRangeF(i)),' i = ',num2str(i),' q = ',num2str(q),' o = ',num2str(o),' k = ',num2str(k),' Miss'])   
% end
end
X(Misses(1:height(X)) == 1,:) = NaN;
SLamb{1}(:,1) = FrequencyRange(1:height(X));
SLamb{1}(:,2) = FrequencyRange(1:height(X))/1e3;
SLamb{1}(:,3) = FrequencyRange(1:height(X))*2*Half;
...
if  HigherOrderModes && any(H)
    ...
    for p = 1:length(H)
        ...
        for i = ceil(H(p)/FrequencyResolution)+1:length(FrequencyRange)
            ...
            for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeReal X SweepRangeImag)
                if  all(X(:,1) == 0) 
                    SweepRangeReal = [1.1*PhaseVelocityLimit Material.PlateVelocity];
                elseif numel(find(X(:,1) ~= 0)) == 1
                    SweepRangeReal = [PhaseVelocityLimit Material.PlateVelocity];
                ...
                if  numel(find(X(:,1) ~= 0)) <= 3 && SweepRangeReal(2) < Material.PlateVelocity
...
% end
        ...
        if  MissingModes(p+1) == 0
            ...
            SLamb{p+1}(:,3) = FrequencyRange(1:height(X))*2*Half;
        ...
        if  max(SLamb{p+1}(:,4))*1e3 > PhaseVelocityLimit
            ...
            PhaseVelocityRange = Max+SSteps:SSteps:PhaseVelocityLimit+SSteps;
            for i = 1:length(PhaseVelocityRange)
                ...
                for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeFrq X SweepRangeImag)
                    if  i == 1
                        SweepRangeFrq = [SLamb{p+1}(MaxInd,1)-SOffset/Half/2e3 SLamb{p+1}(MaxInd,1)+4/Half/2e3];
                        SweepRangeImag = [SearchWidthImag(1)*SLamb{p+1}(MaxInd,8) SearchWidthImag(2)*SLamb{p+1}(MaxInd,8)];
                    else
                        SweepRangeFrq = [X1{p+1}(end-1,1)-SOffset/Half/2e3 X1{p+1}(end-1,1)+4/Half/2e3];

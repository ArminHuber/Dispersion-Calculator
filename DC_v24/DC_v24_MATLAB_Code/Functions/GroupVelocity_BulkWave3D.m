...
% =========================================================================
function GroupVelocity_BulkWave3D(Crop,Material,HeadLine,FontSizeHeadLine,BoxLineWidth,Mode,MarkerSize,ColorBarX,Azimuth,Elevation,Export,PDF,PNG,FileName,Directory,FontSizeAxes,FontSizeAxesLabels,PNGresolution,GroupVelocityCartesian,X3D)   
...
if  HeadLine == 1
    ...
    if  strcmp('L',Mode)
        if  ~strcmp(Material.Class,'Isotropic')
            ax.Title.String = ['Quasilongitudinal wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        else
            ax.Title.String = ['Longitudinal wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        end
    end
    if  strcmp('Ss',Mode)
        if  ~strcmp(Material.Class,'Isotropic')
            ax.Title.String = ['Slow shear wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        else
            ax.Title.String = ['Shear wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        end
    end
    if  strcmp('Sf',Mode)
        if  ~strcmp(Material.Class,'Isotropic')
            ax.Title.String = ['Fast quasishear wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        else
            ax.Title.String = ['Shear wave group velocity in ',char(join(split(Material.Name,'_'),'\_'))];
        end
    end

...
% =========================================================================
function BScholte = Computer_Anisotropic_BScholte(ax,UpperFluid,LowerFluid,ToggleUpperFluid,ToggleLowerFluid,Material,FScholte,H,FrequencyRange,PhaseVelocitySections,PlateThickness,LambPhaseVelocitySweepRange1,LambPhaseVelocitySweepRange2,c,SuperLayerSize,LayerThicknesses,Repetitions,SymmetricSystem,I1,MissingSamples)
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*MINV>
%#ok<*GVMIS> 
...
for i = 2:length(FrequencyRange)
    ...
    for o = 1:PhaseVelocitySections % increase search resolution
        ...
        for k = 1:Bisections % search minimum in characteristic equation and converge upX(1:i-1,1)on it
            ...
            for j = 1:length(WaveNumber)
                ...
                if  ToggleUpperFluid && ToggleLowerFluid
                    ...
                    Y(j) = abs((WLowerFluid-G(4,3)*DLowerFluid)*(WUpperFluid+G(2,1)*DUpperFluid)+G(2,3)*G(4,1)*DUpperFluid*DLowerFluid);
                elseif ToggleUpperFluid && ~ToggleLowerFluid
                    k3UpperFluid = sqrt(AngularFrequency^2/UpperFluid.Velocity^2-WaveNumber2(j));
                    WUpperFluid = k3UpperFluid/WaveNumber(j);
                    DUpperFluid = 1i*UpperFluid.Density*AngularFrequency^2/WaveNumber(j);
                    Y(j) = abs(WUpperFluid+G(2,1)*DUpperFluid);
                elseif ~ToggleUpperFluid && ToggleLowerFluid
                    k3LowerFluid = sqrt(AngularFrequency^2/LowerFluid.Velocity^2-WaveNumber2(j));
                    WLowerFluid = k3LowerFluid/WaveNumber(j);
                    DLowerFluid = 1i*LowerFluid.Density*AngularFrequency^2/WaveNumber(j);
                    Y(j) = abs(WLowerFluid-G(4,3)*DLowerFluid);
                end
            end

...
% =========================================================================
function AScholte = Computer_Anisotropic_AScholte_Coupled_V(Multithreading,Q,ax,Fluid,Material,FrequencyRange,PlateThickness,HigherOrderModes,FScholte,H,FrequencyResolution,Resolution,SearchWidthReal,SearchWidthImag,SearchAreaSections,SearchAreaExtensions,c,SuperLayerSize,LayerThicknesses,Repetitions,Delta,MissingSamples,BelowCutoffWidth)        
if  Resolution > 1e-6
    Resolution = 1e-6;
end
...
for i = 1:length(FrequencyRange)
    ...
    for q = 1:SearchAreaExtensions % q > 1 runs are with extended search area (SweepRangeReal X SweepRangeImag)
        if  i < 4
            SweepRangeReal = [2*FScholte(1) .95*FScholte(1)];
        else
...
AScholte{1}(:,1) = FrequencyRange(1:height(X));
AScholte{1}(:,2) = FrequencyRange(1:height(X))/1e3;
AScholte{1}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
AScholte{1}(:,4) = fillmissing(X(:,1),'spline')/1e3;
AScholte{1}(:,7) = fillmissing(X(:,2),'spline');
AScholte{1}(:,8) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
AScholte{1}(:,9) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
AScholte{1}(AScholte{1}(:,7) < 0,7) = 0; % negative attenuation is impossible
        ...
        for i = ceil(H(p)/FrequencyResolution)+1:length(FrequencyRange)
            ...
            for j = 1:length(AScholte)
                if  i <= height(AScholte{j})
                    Neighbors(j,:) = [AScholte{j}(i,8) AScholte{j}(i,9)];
                end
            end
...
% end
                    ...
                    if  any(MIN)
                        if  (numel(find(X(:,1) ~= 0)) <= 3 && i <= height(AScholte{p}) && AngularFrequency/real(WaveNumber(MIN(1),MIN(2))) < AScholte{p}(i,4)*1e3) ||...
        ...
        if  MissingModes(p+1) == 0
            ...
            X(Misses(1:height(X)) == 1,:) = NaN;
            AScholte{p+1}(:,1) = FrequencyRange(1:height(X));
            AScholte{p+1}(:,2) = FrequencyRange(1:height(X))/1e3;
            AScholte{p+1}(:,3) = FrequencyRange(1:height(X))*PlateThickness;
            AScholte{p+1}(:,4) = fillmissing(X(:,1),'spline')/1e3;
            AScholte{p+1}(:,7) = fillmissing(X(:,2),'spline');
            AScholte{p+1}(:,8) = fillmissing(X(:,3),'spline'); % real phase velocity (m/s)
            AScholte{p+1}(:,9) = fillmissing(X(:,4),'spline'); % imaginary phase velocity (m/s)
            AScholte{p+1}(AScholte{p+1}(:,7) < 0,7) = 0; % negative attenuation is impossible
        else

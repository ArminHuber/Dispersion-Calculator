...
% =========================================================================
function AScholte = Computer_Isotropic_AScholte(Multithreading,Q,ax,Fluid,Material,FAScholte,H,FrequencyRange,PhaseVelocityResolution,PhaseVelocitySections,Half,HigherOrderModes,FrequencyResolution,LambPhaseVelocitySweepRange1,LambPhaseVelocitySweepRange2)        
%#ok<*AGROW>
%#ok<*OR2>
%#ok<*GVMIS> 
global Stop 
...
if  HigherOrderModes && any(H)
    for p = 1:length(H)
        ...
        for i = ceil(H(p)/FrequencyResolution)+1:length(FrequencyRange)
            ...
            if  SweepRange(2) < AScholte{p}(i,4)*1e3+.1
